...
│   ├── architecture-motion.css# 3D-стек архитектурных карточек и Canvas-эффекты
...
│   ├── architecture-motion.js # Кинематика карточек и Canvas-сетка
...
1. **Сохранение видео (Videos are Mandatory):**
   * В секции архитектуры (`#stack-card-1`, `#stack-card-2`, `#stack-card-3`) установлены looping MP4-видео (`assets/telemetry-preview.mp4`, `assets/stream-preview.mp4`, `assets/spectrum-preview.mp4`).
   * В карусели Motion (`.motion-carousel-track`) установлены WebM-видео (`assets/motion-*.webm`).
   * **ЗАПРЕЩЕНО** удалять или заменять теги `<video>` на статические картинки.
